...
export async function activate(context: vscode.ExtensionContext) {
	...
	*/
	...
	// Debug: mark activation complete and list registered commands
	console.log("=== EXTENSION ACTIVATION COMPLETE ===")
	try {
		const all = await vscode.commands.getCommands()
		const filtered = all.filter((c) => c.includes("cline") || c.includes("claude"))
		console.log("Registered commands:", filtered)
	} catch (err: unknown) {
		console.warn("Failed to list commands:", err)
	}
