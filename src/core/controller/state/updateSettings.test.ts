...
describe("updateSettings platform validation", () => {
	...
	it("should allow enabling subagents on macOS (darwin)", async () => {
		...
		(
			mockController.stateManager.getGlobalSettingsKey as sinon.SinonStub
		).returns(false);
		...
		assert.ok(
			(
				mockController.stateManager.setGlobalState as sinon.SinonStub
			).calledWith("subagentsEnabled", true),
	...
	it("should allow disabling subagents on any platform", async () => {
		...
		assert.ok(
			(
				mockController.stateManager.setGlobalState as sinon.SinonStub
			).calledWith("subagentsEnabled", false),
