...
export async function newTask(
	...
	const convertOpenaiReasoningEffort = (
		effort: ProtoOpenaiReasoningEffort,
	): string => {
		switch (effort) {
	...
	const filteredTaskSettings: Partial<Settings> = Object.fromEntries(
		Object.entries({
			...
			...(request.taskSettings?.openaiReasoningEffort !== undefined && {
				openaiReasoningEffort: convertOpenaiReasoningEffort(
					request.taskSettings.openaiReasoningEffort,
				),
			}),
